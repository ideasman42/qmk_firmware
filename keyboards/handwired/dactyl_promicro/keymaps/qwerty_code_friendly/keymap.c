...
[LAYER_BASE] = LAYOUT_dactyl(  // layer 0 : default
  ...
  /* right hand */
        KC_CIRC,        KC_AMPR,        KC_ASTR,        KC_MINS,         KC_EQL,        KC_BSPC,
           ...
           KC_N,           KC_M,        KC_COMM,         KC_DOT,        KC_SLSH,      KC_DELETE,
...
[LAYER_KPAD] = LAYOUT_dactyl(
  /* left hand */
   M_BTICK_PAIR,KC_PRINT_SCREEN, KC_SCROLL_LOCK,   KC_CAPS_LOCK,       KC_PAUSE,        KC_MPRV,
  ...
  M_COLMAK_SWAP,        KC_TRNS,        KC_MINS,        KC_UNDS, M_ARROW_RMINUS,        KC_VOLU,
        ...
        KC_TRNS,        KC_TRNS,        KC_MRWD,        KC_MFFD,        KC_TRNS,
                                                        ...
                                                        KC_TRNS,        KC_TRNS,        KC_TRNS,
  /* right hand */
        KC_TRNS,    KC_NUM_LOCK,    KC_KP_SLASH, KC_KP_ASTERISK,    KC_KP_MINUS,        KC_TRNS,
...
#define WITHOUT_SHIFT(...) \
  do { \
    uint8_t _real_mods = get_mods(); \
    if (_real_mods & MOD_BIT(KC_LSFT)) { SEND_STRING(SS_UP(X_LEFT_SHIFT)); } \
    if (_real_mods & MOD_BIT(KC_RSFT)) { SEND_STRING(SS_UP(X_RIGHT_SHIFT)); } \
    { __VA_ARGS__ } \
    if (_real_mods & MOD_BIT(KC_LSFT)) { SEND_STRING(SS_DOWN(X_LEFT_SHIFT)); } \
    if (_real_mods & MOD_BIT(KC_RSFT)) { SEND_STRING(SS_DOWN(X_RIGHT_SHIFT)); } \
  } while (0)
  ...
  switch (keycode) {
    ...
    case KC_LEFT_SHIFT:  /* '' */
      if (record->event.pressed && (keyboard_report->mods & (MOD_BIT(KC_RSFT)))) {
        WITHOUT_MODS({
            SEND_STRING("''" SS_TAP(X_LEFT) SS_DOWN(X_RIGHT_SHIFT) SS_DOWN(X_LEFT_SHIFT));
          });
        return false;
      break;
    case KC_RIGHT_SHIFT:  /* "" */
      if (record->event.pressed && (keyboard_report->mods & (MOD_BIT(KC_LSFT)))) {
        WITHOUT_MODS({
            SEND_STRING("\x22\x22" SS_TAP(X_LEFT) SS_DOWN(X_LEFT_SHIFT) SS_DOWN(X_RIGHT_SHIFT));
          });
      }
        return false;
      }
